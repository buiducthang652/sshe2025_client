...
export default function NetworksPage({ params: { lng } }) {
  ...
  useEffect(() => {
    async function fetchNetworksNews() {
      try {
        setLoading(true)
        const response = await fetch('/api/categories/w9u0vq1rfhdiyqu6t6cxkl8k')
        
        if (!response.ok) {
          throw new Error('Failed to fetch networks news')
        }
        ...
        console.error('Error fetching networks news:', err)
        setNews([])
        setCategory(null)
      } finally {
        setLoading(false)
      ...
      <div className="container mx-auto px-4">
        <div className="mb-12">
          <h1 className="text-4xl font-bold text-gray-900 mb-4">
            {category?.name || t('news.categories.networks')}
          </h1>
